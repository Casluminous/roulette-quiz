...
export function SlashEffect({ data, onComplete }: SlashEffectProps) {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(() => setPhase('result'), 1400));
    timers.push(setTimeout(() => setPhase('fade'), 2400));
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-none overflow-hidden">
      ...
      {/* Slash trail — vertical line from player avatar to opponent */}
      <AnimatePresence>
        {phase === 'slash' && (
          <motion.div
            key="trail"
            className="absolute z-[109]"
            style={{
              left: slashX,
              width: '3px',
              top: '80%',
      ...
      {/* Knife — starts at player avatar (bottom center), spins, stabs UP through opponent */}
      <AnimatePresence>
      ...
      {/* Per-target results at opponent avatar area */}
      <AnimatePresence>
        {phase === 'result' && targets.map((target) => (
          ...
          >
            {/* Name badge at opponent position */}
            <motion.div
            ...
            {target.hasDied ? (
              /* DEAD: 2 slash marks + X at opponent avatar */
              <>
                <motion.div
                  className="absolute h-[4px]"
                  style={{
                    width: '200px',
                    left: `calc(${slashX} - 100px)`,
                    top: `calc(${slashY} + 12px)`,
